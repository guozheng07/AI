...
![image](https://github.com/user-attachments/assets/4eb6a7af-5177-45a5-9068-0efa83d20529)

![image](https://github.com/user-attachments/assets/eeee24f7-be35-404b-9b32-9d61998faa08)
...
![image](https://github.com/user-attachments/assets/2df31bd2-5c5f-46f1-b219-15567b78b379)

## Transformer 架构原理
### 模型结构
下图出自《Attention Is All You Need》论文，大部分介绍 Transformer 的文章都会引用这张图片，接下来我将带你一步一步揭开这个架构的神秘面纱。

![image](https://github.com/user-attachments/assets/e84a1ad3-3143-4c80-9ff9-d28f397404a0)

在 Transformer 中，编码器可以叫做编码器组，解码器也一样，叫做解码器组，就是说它们不仅仅是由一个编码器和一个解码器组成的。

![image](https://github.com/user-attachments/assets/c99acd6d-6237-44c0-b7fc-ab0fe362e4ea)

![image](https://github.com/user-attachments/assets/f1f76713-8fb3-44b0-92fe-cbcd17f2c760)

### 输入处理（编码器）
![image](https://github.com/user-attachments/assets/85973dae-7184-42e2-95a1-773901177664)

### 自注意层处理（编码器）
![image](https://github.com/user-attachments/assets/aa6cfee3-5464-4aa3-b359-59de78fd5f0a)

### 前馈层处理（编码器）
![image](https://github.com/user-attachments/assets/d20e6a2b-c27a-4c3f-882d-cdca6ccb8d70)

## 小结
这节课我们主要讲解了 Transformer 输入和编码器部分的处理，记住 4 个要点：1+2+1。
- 1 表示位置编码，方便处理序列顺序。
- 2 表示两层：自注意力层和前馈网络层。
- 1 表示每一层进入下一层前都需要进行 Add & Norm 操作。

Transformer 整体看是比较有难度的，这节课我们讲了整体的概念，实际操作起来还是比较复杂的，涉及大量的张量计算、函数计算，我们会在后面的章节中，手把手敲一个 Transformer 模型，到时候可以再进一步深入理解。

### 思考题
这里我给你一个复杂的句子，比如，这部电影不坏，但也不算特别好。Transformer 如何理解这种双重否定句子的情绪偏向？
...
![image](https://github.com/user-attachments/assets/b1155904-1053-436a-9e35-03790f845478)
...
![image](https://github.com/user-attachments/assets/0c24be1b-869e-4e18-804d-f061b9769c9f)

### 编码 - 解码注意力层（解码器）
![image](https://github.com/user-attachments/assets/03f86ed7-b22d-405f-91fe-42ce37e1b882)
...
### Softmax 函数
![image](https://github.com/user-attachments/assets/4048e1fa-5d75-43ba-be3a-8d3cdeaca472)
...
![image](https://github.com/user-attachments/assets/2245f530-9475-44a6-98a7-4a32fc0fe0d1)
...
到这里，Transformer 的基本原理就介绍得差不多了，下面我们看看这么设计有什么好处。

## Transformer 的优势
![image](https://github.com/user-attachments/assets/0520f303-8c05-43ba-9300-f0565b312a6b)

## 小结
Transformer 架构还是比较复杂的，涉及很多概念，对于初学者来说有点难，而且机器学习里的很多概念甚至代码和我们做软件开发相比，完全不一样。不过你不用着急，看不懂没关系，一点一点来，从下一节课开始，我们从 0 到 1 写一个基于 Transformer 架构的模型，相信你会更有感觉。

![image](https://github.com/user-attachments/assets/9e419671-9e20-4b0d-b5f4-bc07d0f31da2)

## 思考题
假设你正在使用一个基于 Transformer 的模型来自动总结新闻文章。你注意到模型在处理某些文章时，重要的信息并没有被有效地捕捉和反映在摘要中，请思考一下，Transformer 模型中的哪一个部分最可能对抓取关键信息并生成准确摘要起到决定性作用？如何简单调整模型，来确保关键信息被捕捉？
